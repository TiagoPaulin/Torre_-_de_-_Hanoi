...
public class Jogo {
    ...
    public void solucaoAutomatica() {
        int id = 1;
        Node aux;
        int ultimaOrigem = -1; // Variável para rastrear a última pilha de origem
        ...
        if ((pilha2.getTopoDado() == null) && (pilha3.getTopoDado() == null)) {
            ...
            if (aux != null) {
                ...
                mostrarPilhas();
                minJogadas++;
                ultimaOrigem = 1; // Atualize a última pilha de origem
            }
        }
        ...
        while (true) {
            ...
            if (aux != null) {
                mostrarPilhas();
                if (id == 3) {
                    if (ultimaOrigem != 1) { // Evite retornar à última pilha de origem
                        autoPush(aux, 1);
                        mostrarPilhas();
                        minJogadas++;
                        ultimaOrigem = 3; // Atualize a última pilha de origem
                    }
                } else {
                    if (ultimaOrigem != (id + 1)) { // Evite retornar à última pilha de origem
                        autoPush(aux, (id + 1));
                        mostrarPilhas();
                        minJogadas++;
                        ultimaOrigem = id; // Atualize a última pilha de origem
                    }
                }
            }
    ...
    public boolean saoIguais(Pilha pilha1, Pilha pilha2) {
        // Verifique se o tamanho das pilhas é igual
        if (pilha1.getTopo() != pilha2.getTopo()) {
            return false;
        }
        Node noPilha1 = pilha1.getPilha();
        Node noPilha2 = pilha2.getPilha();
        // Percorra ambas as pilhas do topo para a base
        while (noPilha1 != null && noPilha2 != null) {
            if (noPilha1.getInformacao() != noPilha2.getInformacao()) {
                return false; // Se os elementos são diferentes, as pilhas não são iguais
            }
            // Avance para o próximo nó em ambas as pilhas
            noPilha1 = noPilha1.getProximo();
            noPilha2 = noPilha2.getProximo();
        }
        // Se chegamos até aqui, todas os elementos foram iguais
        return true;
    }
